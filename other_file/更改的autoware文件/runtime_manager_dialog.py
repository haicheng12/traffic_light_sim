...
class MyFrame(rtmgr.MyFrame):
	...
	def add_config_link(self, dic, panel, obj):
		cfg_obj = wx.adv.HyperlinkCtrl(panel, wx.ID_ANY, '[config]', '')
		fix_link_color(cfg_obj)
		self.Bind(wx.adv.EVT_HYPERLINK, self.OnConfig, cfg_obj)
	...
	def refresh_topics_list(self):
		...
		self.topics_list = []
		for topic in lst:
			obj = wx.adv.HyperlinkCtrl(panel, wx.ID_ANY, topic, '')
			self.Bind(wx.adv.EVT_HYPERLINK, self.OnTopicLink, obj)
	...
	def new_link(self, item, name, pdic, gdic, pnl, link_str, prm_name, add_objs):
		...
		if 'no_link' not in gdic.get('flags', []):
			lkc = wx.adv.HyperlinkCtrl(pnl, wx.ID_ANY, link_str, "")
			if hasattr(lkc, 'SetCanFocus'):
				lkc.SetCanFocus(False)
			fix_link_color(lkc)
			self.Bind(wx.adv.EVT_HYPERLINK, self.OnHyperlinked, lkc)
			if len(add_objs) > 0:
...
def fix_link_color(obj):
	...
	elif t is wx.adv.HyperlinkCtrl:
...
def enables_set(obj, k, en):
	...
	obj_refresh(obj)
	if isinstance(obj, wx.adv.HyperlinkCtrl):
