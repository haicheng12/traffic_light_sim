...
void trafficLightCallback(const std_msgs::Int32ConstPtr &msg)
{
  traffic_light_data = msg->data;
  ROS_WARN("traffic light data is: [%d]", traffic_light_data);
}
...
void TwistCmdCallback(const geometry_msgs::TwistStampedConstPtr &msg)
{
  ...
  //速度赋值，这里增加红绿灯的判断逻辑
  if(traffic_light_data == 1)//红灯为1
  {
    tp.twist.linear.x = 0.0;
    tp.twist.angular.z = lowpass_angular_z;
  }
  else if(traffic_light_data == 2)//绿灯为2
  {
...
int main(int argc, char **argv)
{
    ...
    ros::Subscriber traffic_light_msg_sub = nh.subscribe("traffic_light_msg", 1, trafficLightCallback);;
